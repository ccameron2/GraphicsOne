...
float4 main(LightingPixelShaderInput input) : SV_Target
{
    ...
    float4 textureTV = TVTexture.Sample(TexSampler, input.uv);

    float specularMaterialColour = textureTV.a;   // Specular material colour in texture A (shininess of the surface)
    
	//Make tv image greyscale, wont be using here as texture is a modern tv.
												  
	//float greyScale = (textureColour.r + textureColour.g + textureColour.b) / 3;
    //textureColour.r = greyScale;
    //textureColour.g = greyScale;
    //textureColour.b = greyScale;

    float3 diffuseMaterialColour;

    diffuseMaterialColour = lerp(textureColour.rgb, textureTV.rgb, specularMaterialColour);
